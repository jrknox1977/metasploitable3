...
set RESOURCES_ROOT=%SCRIPTS_ROOT%\..\resources

REM Run scripts in order using relative paths
echo [1/8] Disabling Windows Firewall...
call "%SCRIPTS_ROOT%\configs\disable_firewall.bat"

echo.
echo [2/8] Applying weak password and security settings...
call "%SCRIPTS_ROOT%\configs\apply_password_settings_2019.bat"

echo.
echo [3/8] Creating vulnerable user accounts...
call "%SCRIPTS_ROOT%\configs\create_users.bat"

echo.
echo [4/8] Setting up IIS 10.0 with vulnerable configuration...
call "%SCRIPT_DIR%\setup_iis_2019.bat"

echo.
echo [5/8] Setting up vulnerable FTP server...
call "%SCRIPT_DIR%\setup_ftp_site_2019.bat"

echo.
echo [6/8] Setting up WebDAV with vulnerable configuration...
call "%SCRIPT_DIR%\setup_webdav_2019.bat"

echo.
echo [7/8] Setting up SNMP with weak community strings...
call "%SCRIPT_DIR%\setup_snmp_2019.bat"

echo.
echo [8/8] Setting up Windows 2019 specific vulnerabilities...
call "%SCRIPT_DIR%\setup_win2019_vulns.bat"
